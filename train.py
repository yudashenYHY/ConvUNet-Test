...
from my_dataset_Brain2 import BrainDataset
...
def main(args):
    ...
    train_dataset = datasets.ImageFolder(args.data_path,
                                 ...
                                 transform=get_transform(train=True, crop_size=crop_size, mean=mean, std=std))

    val_dataset = datasets.ImageFolder(args.data_path,

                               transform=get_transform(train=False, crop_size=crop_size, mean=mean, std=std))
...
def parse_args():
    ...
    parser.add_argument("--data-path", default=".\\archive", help="dataset root")
    # exclude background
