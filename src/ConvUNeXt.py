...
class Up(nn.Module):
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        ...
        gate = self.gate(x1).reshape(B, H, W, 3, C).permute(3, 0, 1, 2, 4)#(3,B,H,W,C)
...
class ConvUNeXt(nn.Module):
    def __init__(self,
        ...
        self.down3 = Down(base_c * 4, base_c * 8, layer_num=3)#128，256
        factor = 2 if bilinear else 1
        self.down4 = Down(base_c * 8, base_c * 16 // factor)#256，512
        self.up1 = Up(base_c * 16, base_c * 8 // factor, bilinear)#512，256
